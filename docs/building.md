...
dpkg --info build/transferase-0.6.2-Linux.deb
# Do the install
dpkg --install build/transferase-0.6.2-Linux.deb
...
```console
apt install ./build/transferase-0.6.2-Linux.deb
...
```console
pip install build/python/dist/pyxfr-0.6.2-cp312-none-manylinux_2_38_x86_64.whl
```
...
(`pyxfr-0.6.2-cp312-none-manylinux_2_38_x86_64.whl`) depends on using Ubuntu
...
```console
dnf install build/transferase-0.6.2-Linux.rpm
...
```console
rpm -i build/transferase-0.6.2-Linux.rpm
...
build directory. This command generates a file named `Rxfr_0.6.2.tar.gz`
...
```console
MAKEFLAGS="-j32" R CMD check Rxfr_0.6.2.tar.gz
```
...
me. Now you can use the `Rxfr_0.6.2.tar.gz` as follows to install:
...
* Python package: Notable difference wrt 24.04 is that we use default gcc/g++
  ...
  cmake --build build -j32
  pip install build/python/dist/pyxfr-0.6.2-cp313-none-manylinux_2_38_x86_64.whl
  # Test that it worked
  python3 -c "from pyxfr import *; help(pyxfr)"
  ```
